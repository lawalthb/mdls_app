                                ...
                                <div class="row">
                                    ...
                                    <div class="form-group col-sm-6">
                                        ...
                                        <div id="ctrl-total_score-holder" class=" ">
                                            <input id="ctrl-total_score" data-field="total_score" value="<?php echo get_value('total_score', "0") ?>" type="number" placeholder="Enter Total Score" step="any" required="" name="total_score" class="form-control " v-model="total_score" readonly />
                                        </div>
                                    </div>
                                    <div class="form-group col-sm-6">
                            ...
                            <div class="bg-light p-2 subform">
                                ...
                                <div>
                                    ...
                                    <template id="<?php echo $template_id ?>">
                                        ...
                                        ?>
                                        <tr data-row="<?php echo $row ?>" class="input-row" v-for="(row, index) in rows" :key="index">
                                            <td>
                                                <div id="ctrl-subject_id-row<?php echo $row; ?>-holder" class=" ">
                                                    <select required="" id="ctrl-subject_id-row<?php echo $row; ?>" data-field="subject_id" name="examsheetperformances[<?php echo $row ?>][subject_id]" placeholder="Select a subject ..." class="form-select">
                                                        <option value="">Select a value ...</option>
                                                        <?php
                                            ...
                                            <td>
                                                <div id="ctrl-ca_score-row<?php echo $row; ?>-holder" class=" ">
                                                    <input id="ctrl-ca_score-row<?php echo $row; ?>" data-field="ca_score" value="<?php echo get_value('ca_score', "0") ?>" type="number" placeholder="Enter Ca Score" min="0" max="40" step="0.1" required="" name="examsheetperformances[<?php echo $row ?>][ca_score]" class="form-control " v-model="row.ca_score" :id="'ctrl-ca_score-row' + index" @input="calculateTotal(index)" />
                                                </div>
                                            </td>
                                            <td>
                                                <div id="ctrl-exam_score-row<?php echo $row; ?>-holder" class=" ">
                                                    <input id="ctrl-exam_score-row<?php echo $row; ?>" data-field="exam_score" value="<?php echo get_value('exam_score', "0") ?>" type="number" placeholder="Enter Exam Score" min="0" max="60" step="0.1" required="" name="examsheetperformances[<?php echo $row ?>][exam_score]" class="form-control " v-model="row.exam_score" :id="'ctrl-exam_score-row' + index" @input="calculateTotal(index)" />
                                                </div>
                                            </td>
                                            <td>
                                                <div id="ctrl-total-row<?php echo $row; ?>-holder" class=" ">
                                                    <input id="ctrl-total-row<?php echo $row; ?>" data-field="total" value="<?php echo get_value('total') ?>" type="number" placeholder="Enter Total" min="0" max="100" step="0.1" required="" name="examsheetperformances[<?php echo $row ?>][total]" class="form-control " v-model="row.total" :id="'ctrl-total-row' + index" readonly />
                                                </div>
                                            </td>
                                            <td>
                                            ...
                                            <input id="ctrl-updated_by-row<?php echo $row; ?>" data-field="updated_by" value="<?php echo get_value('updated_by', auth()->user()->id) ?>" type="hidden" placeholder="Enter Updated By" list="updated_by_list" required="" name="examsheetperformances[<?php echo $row ?>][updated_by]" class="form-control " />
                                            <datalist id="updated_by_list">
                                                <?php
                                                $options = $comp_model->updated_by_option_list() ?? [];
                                                foreach ($options as $option) {
                                    ...
                                    <!--[/table row template]-->
                                    <button type="button" @click="addRow" data-template="#<?php echo $template_id ?>" class="btn btn-sm btn-success btn-add-table-row"><i class="material-icons">add</i></button>
                                </div>
                                <div class="form-ajax-status"></div>
                            </div>
                            <!--[form-button-start]-->
...
<script>
    new Vue({
        ...
        data: {
            ...
            total_score: 0 // Field to hold the sum of all total scores
        },
        methods: {
            // Method to calculate the total score for a row and update the total score of all subjects
            calculateTotal(index) {
                let row = this.rows[index];
                row.total = parseFloat(row.ca_score || 0) + parseFloat(row.exam_score || 0);
                this.calculateTotalScore(); // Recalculate the sum of all totals
            },
            // Method to sum all total scores across the rows
            calculateTotalScore() {
                this.total_score = this.rows.reduce((sum, row) => sum + parseFloat(row.total || 0), 0);
            },
            // Method to add a new row dynamically
            addRow() {
                this.rows.push({
                    subject_id: '',
                    ca_score: 0,
